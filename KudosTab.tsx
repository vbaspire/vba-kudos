...
import { Gift, Award, TrendingUp, History, ShieldCheck, ShoppingBag } from 'lucide-react';
...
const KudosTab: React.FC<KudosTabProps> = ({ currentUser }) => {
  ...
  const isSystemUser = (emp: Employee | undefined): boolean =>
    emp?.id === 'system' || emp?.email === 'system@vbaspire.com';
  ...
  const checkMonthlyReset = async (): Promise<void> => {
    try {
      const now = new Date();
      ...
      if (now.getDate() === 1) {
        const { data: balData } = await supabase.from('balances').select('*');
        const needsReset = (balData || []).some((bal: Balance) => {
  ...
  const getUserBalance = (userId: string): Balance => {
    return (
  ...
  const isAdmin = (user: Employee | null): boolean => {
  ...
  const redeemPoints = async (rewardType: 'store' | 'amazon'): Promise<void> => {
  ...
  const getRecentActivity = (): Transaction[] => {
    return transactions.filter((t) => t.receiver_id === currentUser?.id).slice(0, 5);
  };

  const getMostUsedCoreValue = (): { value: string; count: number } | null => {
  ...
  const getLeaderboardData = (): {
    topReceivers: Array<{ employee: Employee | undefined; points: number }>;
    topGivers: Array<{ employee: Employee | undefined; points: number }>;
  } => {
    const currentMonth = new Date().getMonth();
  ...
  const GiveKudosScreen: React.FC = () => {
  ...
  const HistoryScreen: React.FC = () => {
    ...
    const renderTxnDate = (iso: string): string =>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please log in to access VBA Kudos</p>
        </div>
      </div>
    );
  }
      ...
      <nav className="bg-white border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex space-x-1">
            {[
              { id: 'home' as const, label: 'Home', icon: Award },
              { id: 'give' as const, label: 'Give Kudos', icon: Gift },
              ...
              ...(isAdmin(currentUser) ? [{ id: 'admin' as const, label: 'Admin', icon: ShieldCheck }] : []),
